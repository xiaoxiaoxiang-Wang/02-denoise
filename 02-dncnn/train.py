...
def get_model_from_network(channel):
    return network.dncnn(channel)


if __name__=='__main__':
    y_train_val,x_train_val =  data_prepare.get_train_val_data()
    ...
    if os.path.exists(model_path):
        ...
        model = get_model_from_network(y_train_val.shape[-1])
        learning_rate = 0.001
    ...
    plt.figure()
    plt.plot(epochs, history.history['mean_squared_error'], 'b', label='Training mse')
    plt.plot(epochs, history.history['peak_sifnal_to_noise'], 'b', label='Training psnr')
    plt.plot(epochs, history.history['val_mean_squared_error'], 'r', label='Validation mse')
    plt.plot(epochs, history.history['val_peak_sifnal_to_noise'], 'r', label='Validation psnr')
    plt.show()
